...
import { validPushEndpoint, validPushKeys } from "../_shared/push-validation.ts";
...
serve(async (req) => { if (req.method === "OPTIONS") return new Response(null, { headers }); if (req.method !== "POST") return json({ error: "Method not allowed" }, 405); try {
  const { wedding_id, audience_type, guest_session, subscription, platform, user_agent } = await req.json();
  if (typeof wedding_id !== "string" || !uuidPattern.test(wedding_id) || !["guest", "couple"].includes(audience_type)
    || !validPushEndpoint(subscription?.endpoint) || !validPushKeys(subscription?.keys)) return json({ error: "Invalid subscription" }, 400);
  ...
  const { data, error } = await db.from("push_subscriptions").upsert({ wedding_id, audience_type, guest_id: guestId, endpoint: subscription.endpoint, p256dh_key: subscription.keys.p256dh, auth_key: subscription.keys.auth, platform: String(platform || "web").slice(0, 50), user_agent: String(user_agent || req.headers.get("user-agent") || "").slice(0, 500), enabled: true, last_seen_at: new Date().toISOString(), updated_at: new Date().toISOString() }, { onConflict: "wedding_id,endpoint" }).select("id").single();
