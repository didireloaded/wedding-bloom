...
{"theme":"<new theme name>","new_theme":true,"colors":{"primary":"<hsl>","secondary":"<hsl>","accent":"<hsl>","background":"<hsl>","foreground":"<hsl>"},"font_display":"<font name>","font_body":"<font name>","reason":"<brief reason>"}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(JSON.stringify({ error: "Theme service is currently unavailable." }), {
          status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      const t = await response.text();
      console.error("Theme provider error:", response.status, t);
      throw new Error(`Theme provider error: ${response.status}`);
    }
