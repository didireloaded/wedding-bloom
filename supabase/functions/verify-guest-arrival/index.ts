...
import { validLocation, distanceMeters as distance } from "../_shared/arrival-validation.ts";

serve(async (req) => {
  if (req.method === "OPTIONS") return new Response(null, { headers: corsHeaders });
  try {
    const { wedding_id, guest_session, latitude, longitude, accuracy } = await req.json();
    if (!wedding_id || !guest_session || !validLocation(latitude, longitude, accuracy)) return json({ error: "Missing location or guest session" }, 400);
    ...
    if (verification_token) { const { error } = await db.from("guest_sessions").update({ verification_token, verification_expires_at: new Date(Date.now() + 5 * 60 * 1000).toISOString(), last_seen_at: new Date().toISOString() }).eq("id", session.id); if (error) return json({ error: "Could not save arrival verification" }, 500); }
