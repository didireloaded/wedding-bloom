...
export default function GuestNotificationInbox({ weddingId, guestSession, items, onRefresh, onAction }: {
  ...
  return <section className="guest-inbox mx-auto mb-4 w-full max-w-xl px-5">
    <button onClick={() => setOpen(!open)} className="guest-inbox-toggle flex min-h-14 w-full items-center gap-3 p-4 text-left" aria-expanded={open}>
    ...
    {open && <div className="mt-3 space-y-2">{items.length === 0 ? <p className="p-3 text-sm text-muted-foreground">No updates yet.</p> : items.map(item => <button disabled={busy !== null} key={item.id} onClick={() => void visit(item)} className="guest-inbox-item flex w-full gap-3 p-4 text-left disabled:opacity-60">
