...
export default function NotificationPreferences({ weddingId }: { weddingId: string }) {
  ...
  return <div className="rounded-[24px] border border-border bg-card p-5 shadow-sm"><div className="mb-4 flex items-center gap-3"><div className="rounded-full bg-[#202020] p-2 text-white"><Bell className="h-4 w-4" /></div><div><p className="font-body text-sm font-semibold">Notification preferences</p><p className="font-body text-xs text-muted-foreground">Choose what reaches you on this device.</p></div></div><div className="space-y-3">{categories.map((category) => <div key={category} className="flex items-center justify-between gap-3"><span className="font-body text-sm capitalize">{category}</span><select value={modes[category]} disabled={saving === category} onChange={(event) => update(category, event.target.value)} className="rounded-full border border-border bg-muted/40 px-3 py-2 font-body text-xs"><option value="immediate">Immediate</option><option value="grouped">Grouped</option><option value="off">Off</option></select></div>)}</div></div>;
}
