...
export default function ReminderHistory({ weddingId }: { weddingId: string }) {
  ...
  return <section className="space-y-3 font-body"><h3 className="text-lg font-semibold">Recent reminders</h3>
    {query.error ? <p role="alert" className="text-sm">Reminders could not be loaded. <button className="underline" onClick={() => void query.refetch()}>Retry</button></p> : query.isLoading ? <p className="text-sm">Loading reminders...</p> : !query.data?.length ? <p className="text-sm text-muted-foreground">No reminders sent yet.</p> : <ul className="divide-y divide-black/5">{query.data.map(item => <li key={item.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="break-words text-sm font-medium">{item.rsvps?.guest_name || 'Guest'}</p><p className="mt-1 text-xs text-muted-foreground">{new Date(item.created_at).toLocaleString()}</p></div><span className="shrink-0 rounded-full bg-white px-3 py-2 text-xs">{item.read_at ? 'Opened' : 'In guest inbox'}</span></li>)}</ul>}
  </section>;
}
