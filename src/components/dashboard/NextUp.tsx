...
export default function NextUp({ weddingId, onTabChange }: { weddingId: string; onTabChange: (tab: string) => void }) {
  ...
  return <section className="space-y-3 font-body">
    ...
    {adding && <form onSubmit={save} className="space-y-3 rounded-3xl bg-card p-4">
    ...
    {sorted.map(row => <article key={row.id} className="flex items-center gap-2 rounded-3xl bg-card p-3">
