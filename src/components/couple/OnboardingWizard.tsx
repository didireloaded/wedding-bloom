...
function PhotoField({ preview, onChoose, onRemove }: { preview: string; onChoose: (file?: File) => void; onRemove: () => void }) { return <div><p className="mb-2 font-body text-xs font-semibold">Couple photo</p>{preview ? <div className="relative h-56 overflow-hidden rounded-2xl bg-black"><img src={preview} alt="" aria-hidden="true" className="absolute -inset-4 h-[calc(100%+32px)] w-[calc(100%+32px)] object-cover blur-xl opacity-60" /><img src={preview} alt="Selected couple" className="relative h-full w-full object-contain" /><button type="button" aria-label="Remove photo" onClick={onRemove} className="absolute right-3 top-3 rounded-full bg-white p-2 shadow"><X className="h-4 w-4" /></button></div> : <label className="flex h-56 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-black/15 bg-white text-center"><ImagePlus className="mb-3 h-7 w-7" /><span className="font-body text-sm font-medium">Add a couple photo</span><span className="mt-1 font-body text-xs text-muted-foreground">JPG, PNG, or WEBP up to 8 MB</span><input type="file" accept="image/*" onChange={(e) => onChoose(e.target.files?.[0])} className="sr-only" /></label>}</div>; }
