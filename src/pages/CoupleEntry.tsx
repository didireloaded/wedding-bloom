...
const styleBySlug: Record<string, string> = {
  "elara-julian": "Garden Estate Wedding",
  "towa-mathew": "Luxury Garden Wedding",
  "john-anna": "French Riviera Wedding",
  "emma-david": "Wildflower Farm Wedding",
  "lisa-michael": "Winter Glasshouse Wedding",
};

const cityBySlug: Record<string, string> = {
  "elara-julian": "Tuscany, Italy",
  "towa-mathew": "Yokohama, Japan",
  "john-anna": "Cote d'Azur, France",
  "emma-david": "Virginia, USA",
  "lisa-michael": "Manchester, UK",
};
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        ...
        {mode === "explore" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h2 className="display text-[34px] text-[#221e1b] mb-6">Inspiration from real designs.</h2>
                ...
                <button key={w.id} onClick={() => navigate(`/wedding/${w.slug}?preview=1`)} className="text-left bg-white rounded-[24px] border border-[#e6d4be] overflow-hidden hover:shadow-xl transition-all">
                  <div className="relative aspect-[16/9]">
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      ...
                      <div className="text-white/80 text-[11px] uppercase tracking-[0.18em] mt-1">{styleBySlug[w.slug] || "Wedding"}</div>
                    </div>
                  </div>
                  <div className="p-4 text-[13px] text-[#6b5d4f] flex items-center justify-between">
                    <span>{cityBySlug[w.slug] || w.ceremony_venue}</span>
        ...
        {(mode === "signin" || mode === "signup") && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto bg-white rounded-[28px] border border-[#e6d4be] p-6 md:p-8">
            ...
            <div className="mt-5 text-center text-[12.5px] text-[#8d7962]">
              Test code for this wedding: <span className="text-[#b0743c] font-semibold tracking-[0.1em]">{wedding.access_code}</span>
            </div>
          </motion.div>
        )}
      </div>
